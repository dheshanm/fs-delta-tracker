...
def start_scan_log(
    ...
    """
    Adds a file handler at LOG_ROOT/scans/{scan_id}.log
    ...
    """
    log_params = config(config_file, "logging")
    log_root = log_params["log_root"]

    if log_root.startswith("/"):
    ...
    log_file = log_root / "scans" / f"{scan_id}.log"
    ...
    logging.getLogger().addHandler(file_handler)
    logger.info(f"Logging scan {scan_id} to {log_file}")
